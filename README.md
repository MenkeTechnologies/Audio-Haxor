...
| **`test/i18n-anchor-keys.test.js`** | 120 (twenty `menu.*` / `tray.*` anchors × six non-`en` locales; one `test()` per pair) | Each anchor’s value in `de`/`es`/`fr`/`nl`/`pt`/`sv` must not equal English (catches pasted `en` rows); complements Rust `seed_json_*_*_differs_from_en` and `i18n-seed-parity.test.js`. |
