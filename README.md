...
| **Audio Engine tab** | Shell UI for **future** dedicated audio engine: separate process for low-latency system output, **output device** selection, and **insert plugin** graph. **F6** (`tab14` shortcut) opens the tab; View menu and command palette include it. **Rust:** `src-tauri/src/audio_engine.rs` is a stub (`AudioEngineStub`); real engine will use IPC / sidecar. **JS:** `frontend/js/audio-engine.js` — `initAudioEngineTab()` on tab activation; status text uses `ui.ae.*` strings until the backend exists |
...
frontend/
  ...
  js/
    ...
    audio-engine.js    -- Audio Engine tab shell (future engine process / device / plugin graph)
