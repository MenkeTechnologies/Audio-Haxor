...
**Rust cache (CI):** [Swatinem/rust-cache](https://github.com/Swatinem/rust-cache) must use `workspaces: .` (repo root). The virtual `[workspace]` writes **`target/`** at the repository root (`cargo metadata` → `target_directory`); `workspaces: src-tauri` would cache **`src-tauri/target`**, which this layout does not use and can misalign CI artifacts (e.g. Windows test runs). **Rust tests** use the same **`cargo test --manifest-path src-tauri/Cargo.toml`** on **ubuntu-latest**, **macos-latest**, and **windows-latest**. An older Windows-only flow ran **`cargo test --no-run`**, deleted **every** **`*.dll`** under **`target/debug`**, then ran each test exe manually to avoid JUCE/VC sidecars shadowing the loader (`STATUS_ENTRYPOINT_NOT_FOUND`); that stripped DLLs the harnesses still need and was removed.
