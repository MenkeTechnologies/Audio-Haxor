...
| **Audio Engine tab** | **Sidecar** `audio-engine` (cpal): **persistent stdin loop** (one child process reused by the app), **stable device ids**, **start_output_stream** / **stop_output_stream** (silence buffer — placeholder for real mixer), **output_stream_status**. **F6** opens the tab. **Rust:** `audio_engine_invoke` (`src-tauri/src/audio_engine.rs`). **Dev:** `beforeDevCommand` builds `audio-engine`. **Release:** `externalBin` + `scripts/prepare-audio-engine-sidecar.mjs`. **JS:** `frontend/js/audio-engine.js` — prefs `audioEngineOutputDeviceId` |
