...
**Library playback (`start_playback`):** **`playback_load`** probes the file with symphonia so **`src_rate`** matches the first decoded packet when **`codec_params`** from the container does not. **`start_output_stream`** with **`start_playback: true`** opens **[rodio](https://github.com/RustAudio/rodio)** on the selected device and prefers the same **F32** sample rate / stereo layout as before. **`PlaybackShared`** replaces the previous **`Player`** when a new load or stream restart runs so detached playback cannot continue after **Stop** or a new load. **`buffer_frames`** is **capped at 8192** in the sidecar (and in **`audio-engine.js`** prefs) so mistaken values like `144000` do not create multi‑second hardware buffers that keep playing after **Stop stream**. After **Stop stream** clears the sidecar session, **Apply** runs **`playback_load`** from the last engine preview path so **`start_playback`** works again. The frontend seeds duration from **`playback_load`**’s **`duration_sec`** (and falls back to the scanner’s stored duration) for waveform seek and progress UI so clicks work before the first **`playback_status`** poll; engine mute uses the same **`prefs.audioVolume`** path as the slider for **`playback_set_dsp`**. **Speed** maps to **`playback_set_speed`** (rodio pitch behavior, same as `<audio>.playbackRate`); **reverse** uses **`playback_set_reverse`** (RAM-heavy full decode) and **`stop_output_stream` → `start_output_stream`** so the rodio source is rebuilt. See `audio-engine/README.md`.
