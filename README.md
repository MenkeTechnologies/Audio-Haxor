...
| **Command Palette** | Press <kbd>Cmd+K</kbd> to open a fuzzy search across tabs, actions, bookmarked directories, and tags. The large tab/action/toggle list is **built once per locale** (cached; cleared when UI strings reload) and merged with bookmarks, tags, and player-related rows on each open. For queries of **2+ characters**, in-memory matches render at once; inventory hits (plugins, samples, DAW projects, presets, PDFs, MIDI) then merge in from **SQLite** via **`db_query_palette_preview`** (one IPC round-trip, six `LIMIT 6` queries on the blocking pool) — not from in-memory arrays. Arrow keys to navigate, Enter to select, Escape to dismiss. Uses the same fzf scoring engine as tab search bars |
