...
| **Audio Engine tab** | **Sidecar** `audio-engine` (**JUCE** 8 — device I/O, file decode, **VST3 + AU** plugin scan via `KnownPluginList`; same stdin/stdout JSON IPC as before): **engine_state**, **list_input_devices**, **get_input_device_info** / **get_output_device_info**, **set_input_device** / **set_output_device**, **start_output_stream** / **stop_output_stream** (optional **`start_playback`** after **`playback_load`** for library PCM + 3‑band EQ / gain / pan in the sidecar), **playback_*** status/seek/DSP, **start_input_stream** / **stop_input_stream**, **set_output_tone** (F32 **440 Hz** test sine), **buffer_size** / **stream_buffer_frames**, **`plugin_chain`** (scanned plugin list; real-time insert graph planned). **F6** opens the tab. **Rust:** `audio_engine_invoke` (`src-tauri/src/audio_engine.rs`). **Dev:** `node scripts/build-audio-engine.mjs` (CMake + Ninja → `target/debug/audio-engine`). **Release:** `externalBin` + `scripts/prepare-audio-engine-sidecar.mjs`. **JS:** `frontend/js/audio-engine.js`. JUCE is **GPLv3 / commercial license** — see `audio-engine/README.md`. |
