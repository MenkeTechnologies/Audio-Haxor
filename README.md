...
| **Audio Engine tab** | **Sidecar** `audio-engine` (cpal): **persistent stdin loop**, **start_output_stream** / **stop_output_stream** (silence — mixer placeholder), **output_stream_status** (running stream reports **sample_rate_hz**, **channels**, **sample_format**, **buffer_size**, **device_name**). **F6** opens the tab. **Rust:** `audio_engine_invoke` (`src-tauri/src/audio_engine.rs`). **Dev:** `beforeDevCommand` builds `audio-engine`. **Release:** `externalBin` + `scripts/prepare-audio-engine-sidecar.mjs`. **JS:** `frontend/js/audio-engine.js` — prefs `audioEngineOutputDeviceId` |
