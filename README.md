...
| **Scan preserves visible rows** | Starting a scan (any inventory tab or **Scan All**) no longer replaces the current grid with a spinner or empty state immediately — the existing SQLite-backed table stays on screen until the first streamed batch arrives (then streaming append/replace behavior applies as before). Plugins clear the card list only on the first `scanning` progress event |
