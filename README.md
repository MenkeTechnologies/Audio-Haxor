...
frontend/
  index.html           -- Cyberpunk CRT UI (HTML/CSS); tab strip uses **command-palette** tab icons (left of label, `data-i18n` on inner `<span>` so glyphs are not overwritten); `.audio-toolbar > .btn` min-height aligns scan/update toolbars
  js/
