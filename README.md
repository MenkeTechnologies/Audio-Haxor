...
| **`test/i18n-appfmt.test.js`** | tally `rg -c '^test\\(' test/i18n-appfmt.test.js` | `appFmt` / `toastFmt` placeholder contract (mirrors `frontend/js/ipc.js`: missing key → key, empty value → key, `\{(\w+)\}` interpolation). |
