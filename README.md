...
| **Export/Import** | Export all tabs (plugins, samples, DAW projects, presets) to JSON, TOML, CSV, or TSV via native file dialogs. Import from JSON or TOML. Format auto-detected from file extension. **DAW export** uses `dbDawFilterStats` / a one-row `dbQueryDaw` probe when in-memory totals are zero but the library has projects (fixes “Nothing to export” with a full table) |
