...
| **`test/i18n-anchor-keys.test.js`** | 24 (four `menu.*` / `tray.*` anchors × six non-`en` locales; one `test()` per pair) | Each anchor’s value in `de`/`es`/`fr`/`nl`/`pt`/`sv` must not equal English (catches pasted `en` rows); complements Rust `seed_json_*_menu_scan_all_differs_from_en` and `i18n-seed-parity.test.js`. |
...
**Total `node:test` count:** run `pnpm run test:js` and read the `ℹ tests N` line (authoritative; the table above does not sum to `N` because some suites use `describe`/`it` or generated `test()` loops).
