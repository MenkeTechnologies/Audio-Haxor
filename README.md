...
| **Audio Engine tab** | **Sidecar process** `audio-engine` (cpal): lists **output devices**, validates **Apply**, stub **plugin_chain** for future inserts. **F6** opens the tab. **Rust:** `audio_engine_invoke` spawns the binary next to the app (`src-tauri/src/audio_engine.rs`); JSON one line stdin → one line stdout. **Release:** `bundle.externalBin` + `scripts/prepare-audio-engine-sidecar.mjs` (runs as `beforeBuildCommand`). **JS:** `frontend/js/audio-engine.js` — device `<select>`, refresh, prefs key `audioEngineOutputDeviceId` |
