 ...
 * - `seed_json_appfmt_placeholders_preserved_de_el_fr_it_nl_pl_pt_sv` (Rust; here: strict token multiset parity for `de`/`el`/`fr`/`it`/`nl`/`pl`/`pt`/`sv`)
...
test('de, el, fr, it, nl, pl, pt, sv: appFmt placeholder token multiset matches English for every key', () => {
  const en = loadMap('app_i18n_en.json');
  for (const loc of ['de', 'el', 'fr', 'it', 'nl', 'pl', 'pt', 'sv']) {
