 ...
 * - `seed_json_appfmt_placeholders_preserved_…_uk_vi` (Rust; here: strict token multiset parity for `de`/`el`/`es`/`fi`/`fr`/`it`/`nl`/`pl`/`pt`/`ru`/`sv`/`zh`/`ja`/`ko`/`da`/`nb`/`tr`/`cs`/`hu`/`ro`/`uk`/`vi`; JS multiset is stricter than Rust substring check)
...
test('de, el, es, fi, fr, it, nl, pl, pt, ru, sv, zh, ja, ko, da, nb, tr, cs, hu, ro, uk, vi: appFmt placeholder token multiset matches English for every key', () => {
