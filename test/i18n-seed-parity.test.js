 ...
 * - `seed_json_appfmt_placeholders_preserved_de_fr_nl_pt_sv` (Rust; here: strict token multiset parity for `de`/`fr`/`nl`/`pt`/`sv`)
...
test('de, fr, nl, pt, sv: appFmt placeholder token multiset matches English for every key', () => {
  const en = loadMap('app_i18n_en.json');
  for (const loc of ['de', 'fr', 'nl', 'pt', 'sv']) {
