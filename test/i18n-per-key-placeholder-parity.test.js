 ...
 * - `de` / `fr` / `it` / `nl` / `pt` / `sv`: IPC `{token}` multiset matches English for every key that uses placeholders
...
const localeMaps = { de, fr, it, nl, pt, sv };

for (const loc of /** @type {const} */ (['de', 'fr', 'it', 'nl', 'pt', 'sv'])) {
