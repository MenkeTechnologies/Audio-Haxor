...
## Other locales (`cs` — Czech, `da`, `de`, `es`, `sv`, `fr`, `nl`, `pt`, `it`, `el`, `pl`, `ru`, `zh` — Simplified Chinese, `ja` — Japanese, `ko` — Korean, `fi` — Finnish, `nb` — Norwegian Bokmål, `tr` — Turkish, `hu` — Hungarian, `ro` — Romanian, `uk` — Ukrainian, `vi` — Vietnamese)
...
If English already contains new keys and you need the same keys in `de`/`es`/`sv`/`fr`/`nl`/`pt`/`it`/`el`/`pl`/`ru`/`zh`/`ja`/`ko`/`fi`/`da`/`nb`/`tr`/`cs`/`hu`/`ro`/`uk`/`vi` with English placeholder text until a full `gen_app_i18n_*` run:
...
- `test/i18n-anchor-keys.test.js` — for keys where **cs/da/de/el/es/fi/fr/hu/it/ja/ko/nb/nl/pl/pt/ro/ru/sv/tr/uk/vi/zh** all differ from English, none of those locales may copy `en` verbatim.
