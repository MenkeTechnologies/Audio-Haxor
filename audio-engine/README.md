...
| `playback_set_inserts` | `paths` (JSON array, max 8, empty clears): each entry is either an on-disk **`.vst3` bundle** / **`.component`**, or the same **`fileOrIdentifier`** string the UI got from **`plugin_chain`** **`plugins`** (e.g. **`AudioUnit:…`** — not a filesystem path). | Load **VST3** / **AU** (macOS) effect instances in order. **Requires** `stop_output_stream` first (no hot-swap while the device is open). Closes any open insert editor windows. |
