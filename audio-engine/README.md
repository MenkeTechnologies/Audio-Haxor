...
These tests spawn the binary and assert **stdin/stdout JSON** (`ping`, bad JSON handling, **`playback_load`** / **`waveform_preview`** / **`spectrogram_preview`** path validation — **no** `list_*_devices` / `plugin_chain`, which can block in piped shells). Override the binary path with **`AUDIO_ENGINE_TEST_BIN`**. On **Linux** without a display, run under **`xvfb-run -a`** (as in CI).
