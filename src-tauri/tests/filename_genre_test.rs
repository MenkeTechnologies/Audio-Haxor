use app_lib::sample_analysis::{analyze_sample, detect_manufacturer, match_category};
...
fn test_filename_genre_scoring_simulation() {
    ...
    let score_techno = techno_keywords
        .iter()
        .filter(|kw| filename_techno.to_lowercase().contains(*kw))
        .count();
    assert!(score_techno >= 2);

    let score_trance = trance_keywords
        .iter()
        .filter(|kw| filename_trance.to_lowercase().contains(*kw))
        .count();
    assert!(score_trance >= 2);

    let score_schranz = schranz_keywords
        .iter()
        .filter(|kw| filename_schranz.to_lowercase().contains(*kw))
        .count();
...
fn test_genre_specific_exclusions_in_filenames() {
    ...
    let is_bad = BAD_GENRES
        .iter()
        .any(|bad| filename.to_lowercase().contains(bad));
    assert!(is_bad);
}
