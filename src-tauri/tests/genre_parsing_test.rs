use app_lib::sample_analysis::{analyze_sample, detect_manufacturer};
use app_lib::sample_filters::{
    BAD_GENRES, BAD_GENRES_SCHRANZ, BAD_GENRES_TRANCE, is_ableton_project_sample,
};
...
fn test_genre_detection_from_manufacturer_signals() {
    ...
    let m =
        detect_manufacturer("/Samples/Trance/Armada/Pack1/").expect("Armada should be detected");
    assert!(m.genre_score > 0.0, "Armada should lean toward trance");
    assert_eq!(m.manufacturer_pattern, "Armada");

    let m = detect_manufacturer("/Users/wizard/Samples/Anjuna/Sounds/")
        .expect("Anjuna should be detected");
    assert!(m.genre_score > 0.0, "Anjuna should lean toward trance");

    // Techno leaning (negative genre_score)
    let m = detect_manufacturer("/Volumes/Storage/Drumcode/Essentials/")
        .expect("Drumcode should be detected");
    ...
    let m = detect_manufacturer("/Samples/Schranz/Schranz Total/")
        .expect("Schranz Total should be detected");
    assert!(
        m.genre_score <= -0.9,
        "Schranz Total should lean heavily toward techno/schranz"
    );
...
fn test_bad_genre_keywords() {
    ...
    assert!(
        BAD_GENRES
            .iter()
            .any(|g| path_samba.to_lowercase().contains(g))
    );
    ...
    assert!(
        BAD_GENRES
            .iter()
            .any(|g| path_hiphop.to_lowercase().contains(g))
    );
...
fn test_ableton_project_sample_detection() {
    assert!(is_ableton_project_sample(
        "/Users/wizard/Music/My Project/Samples/Imported/Kick.wav"
    ));
    assert!(is_ableton_project_sample(
        "C:\\Music\\Test Project\\Samples\\Processed\\Lead.wav"
    ));
    assert!(!is_ableton_project_sample(
        "/Users/wizard/Music/Samples/Techno/Kick.wav"
    ));
}
