...
struct SongSamples {
    ...
    kick_rumbles: Vec<Vec<SampleInfo>>,
    kick_noises: Vec<Vec<SampleInfo>>,
    hardcore_kicks: Vec<Vec<SampleInfo>>,
...
fn generate_swoosh_arrangements() -> Vec<TrackArrangement> {
    ...
    with_gen_rng(|rng| {
        ...
        let grid_positions: Vec<u32> =
            vec![16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208];
...
fn generate_scatter_hits(
    section_scatter: &SectionValues,
    global_scatter: f32,
    track_count: u32,
) -> Vec<TrackArrangement> {
    with_gen_rng(|rng| {
        ...
        for track_num in 1..=n {
            ...
            if !sections_out.is_empty() {
                ...
                results.push(TrackArrangement::new(
                    &format!("SCATTER {}", track_num),
                    sections_out,
                ));
            }
        }
...
fn generate_glitch_arrangements() -> Vec<TrackArrangement> {
    ...
    with_gen_rng(|rng| {
        ...
        let num_tracks = 8;
        let mut track_sections: Vec<Vec<(f64, f64)>> =
            (0..num_tracks).map(|_| Vec::new()).collect();
        ...
        for (i, sections) in track_sections.into_iter().enumerate() {
            if !sections.is_empty() {
                arrangements.push(TrackArrangement::new(
                    &format!("GLITCH {}", i + 1),
                    sections,
                ));
            }
...
fn get_arrangement(chaos: f32) -> Vec<TrackArrangement> {
    ...
    let mut base = vec![
        ...
        TrackArrangement::new("PAD 2", vec![(81.0, 88.5), (89.0, 96.0)]),
        ...
        TrackArrangement::new(
            "KEYS 2",
            vec![(73.0, 80.0), (81.0, 88.5), (145.0, 152.5), (153.0, 160.0)],
        ),
        ...
        TrackArrangement::new(
            "SUB DROP 1",
            vec![(65.0, 65.0), (73.0, 73.0), (81.0, 81.0), (89.0, 89.0)],
        ),
        TrackArrangement::new("SUB DROP 2", vec![(73.0, 73.0), (81.0, 81.0), (89.0, 89.0)]),
        TrackArrangement::new("SUB DROP 3", vec![(81.0, 81.0), (89.0, 89.0)]),
        TrackArrangement::new("SUB DROP 4", vec![(89.0, 89.0)]),
        ...
        TrackArrangement::new(
            "BOOM KICK 2",
            vec![(73.0, 73.0), (81.0, 81.0), (89.0, 89.0)],
        ),
        TrackArrangement::new("BOOM KICK 3", vec![(81.0, 81.0), (89.0, 89.0)]),
        TrackArrangement::new("BOOM KICK 4", vec![(89.0, 89.0)]),
        ...
        // === KICK LAYERS === (rumble + noise follow kick timing — they ARE the kick)
        // KICK RUMBLE: sub/drive layer, runs with kick but enters slightly later
        TrackArrangement::new(
            "KICK RUMBLE",
            vec![
                // INTRO - enters bar 9 (kick enters bar 1)
                (9.0, 16.75),
                (17.0, 24.5),
                (25.0, 32.0),
                // BUILD
        ...
        // KICK NOISE: distortion/texture top layer, enters later for energy builds
        TrackArrangement::new(
            "KICK NOISE",
            vec![
                // BUILD - enters bar 33
                ...
                (145.0, 152.5),
                (153.0, 160.0),
                // FADEDOWN - drops out at 177
                (161.0, 168.75),
                (169.0, 176.0),
            ],
        ),
        // HARDCORE KICK: transitional percussion, appears at energy peaks
        TrackArrangement::new(
            "HARDCORE KICK",
            vec![
                (49.0, 64.0),   // build peak
                (113.0, 128.0), // drop 1 second half
                (145.0, 160.0), // drop 2 second half
            ],
        ),
        // === BREAKBEAT === (transitional — before drops, during breakdowns, build peaks)
        TrackArrangement::new(
            "BREAKBEAT",
            vec![
                (57.0, 64.0),   // end of build → into breakdown
                (89.0, 96.0),   // end of breakdown → into drop 1
                (153.0, 160.0), // end of drop 2 → into fadedown
                (185.0, 192.0), // end of fadedown → into outro
            ],
        ),
        // === ATMOSPHERE ===
        TrackArrangement::new(
        ...
        TrackArrangement::new("ATMOS 2", vec![(65.0, 96.0), (129.0, 160.0)]),
        TrackArrangement::new("VOX 1", vec![(81.0, 96.0), (113.0, 128.0), (145.0, 160.0)]),
    ];
...
fn get_arrangement_with_params(
    chaos: f32,
    ...
    if has_any_parallelism {
        arrangements = apply_parallelism_per_section(
            arrangements,
            &section_overrides.parallelism,
            parallelism,
            &section_overrides.variation,
            variation,
        );
    }
    ...
    if has_any_variation {
        arrangements =
            apply_variation_per_section(arrangements, &section_overrides.variation, variation);
    }
    ...
    if has_any_scatter {
        arrangements.extend(generate_scatter_hits(
            &section_overrides.scatter,
            scatter,
            scatter_track_count,
        ));
    }
...
fn apply_parallelism(
    arrangements: Vec<TrackArrangement>,
    section_parallelism: &SectionValues,
    global_parallelism: f32,
    section_variation: &SectionValues,
    global_variation: f32,
) -> Vec<TrackArrangement> {
    ...
    for (_base_type, indices) in groups.iter() {
        ...
        let mut track_active_slots: Vec<Vec<bool>> =
            vec![vec![false; num_slots as usize]; group_size];
        ...
        with_gen_rng(|rng| {
            ...
            for slot in 0..num_slots as usize {
                ...
                let parallelism =
                    section_parallelism.value_at_bar(slot_bar.max(1), global_parallelism);
                let max_concurrent = ((group_size as f32 * parallelism).ceil() as usize)
                    .max(1)
                    .min(group_size);
...
fn apply_variation(
    mut arrangements: Vec<TrackArrangement>,
    section_variation: &SectionValues,
    global_variation: f32,
) -> Vec<TrackArrangement> {
    with_gen_rng(|rng| {
        ...
        for arr in arrangements.iter_mut() {
            ...
            for &(start, end) in &arr.sections {
                ...
                let variation =
                    section_variation.value_at_bar(((start + end) / 2.0) as u32, global_variation);
                ...
                while pos < end {
                    ...
                    if rng.random_bool(play_prob) {
                        ...
                        if actual_end > pos {
                            new_sections
                                .push((snap_4bar(pos).max(start), snap_4bar(actual_end).min(end)));
                        }
                    }
...
fn apply_chaos_to_arrangements(
    mut arrangements: Vec<TrackArrangement>,
    section_chaos: &SectionValues,
    global_chaos: f32,
) -> Vec<TrackArrangement> {
    with_gen_rng(|rng| {
        ...
        for arr in arrangements.iter_mut() {
            ...
            if call_response_prefixes
                .iter()
                .any(|p| arr.name.starts_with(p))
            {
                let has_number = arr
                    .name
                    .chars()
...
fn apply_parallelism_per_section(
    arrangements: Vec<TrackArrangement>,
    section_parallelism: &SectionValues,
    global_parallelism: f32,
    section_variation: &SectionValues,
...
fn apply_variation_per_section(
    arrangements: Vec<TrackArrangement>,
    section_variation: &SectionValues,
    global_variation: f32,
) -> Vec<TrackArrangement> {
...
fn apply_density_per_section(
    mut arrangements: Vec<TrackArrangement>,
    section_density: &SectionValues,
    global_density: f32,
) -> Vec<TrackArrangement> {
    with_gen_rng(|rng| {
...
fn apply_glitch_edits(
    mut arrangements: Vec<TrackArrangement>,
    glitch_intensity: f32,
    section_glitch: &SectionValues,
) -> Vec<TrackArrangement> {
    ...
    let has_any_glitch = glitch_intensity > 0.05 || section_glitch.values().any(|v| v > 0.05);
...
impl SampleInfo {
        ...
        let duration_secs = if db_duration >= 0.5 {
            ...
            if let Some(sample_bpm) = bpm {
                if name.to_lowercase().contains("loop") {
                    ...
                    let bars = if estimated_bars <= 1.5 {
                        1.0
                    } else if estimated_bars <= 3.0 {
                        2.0
                    } else if estimated_bars <= 6.0 {
                        4.0
                    } else if estimated_bars <= 12.0 {
                        8.0
                    } else {
                        estimated_bars.min(32.0)
                    };
    ...
    fn is_loop(&self, _project_bpm: f64) -> bool {
        ...
        if path_lower.contains("/one-shots/")
            || path_lower.contains("\\one-shots\\")
            || path_lower.contains("/oneshots/")
            || path_lower.contains("\\oneshots\\")
            || path_lower.contains("/one_shots/")
            || path_lower.contains("\\one_shots\\")
            || path_lower.contains("/one-shot/")
            || path_lower.contains("\\one-shot\\")
            || path_lower.contains("/hits/")
            || path_lower.contains("\\hits\\")
            || path_lower.contains("_hits/")
            || path_lower.contains("_hits\\")
            || path_lower.contains("/drum_hits/")
            || path_lower.contains("\\drum_hits\\")
        ...
        if name_lower.contains("one_shot")
            || name_lower.contains("one-shot")
            || name_lower.contains("oneshot")
            || name_lower.contains("one shot")
            || name_lower.contains("_hit_")
            || name_lower.contains("_hit.")
            || name_lower.contains("_shot_")
            || name_lower.contains("_shot.")
            || name_lower.contains("_stab_")
            || name_lower.contains("_stab.")
        {
            return false;
        }
        ...
        if path_lower.contains("/loops/")
            || path_lower.contains("\\loops\\")
            || path_lower.contains("/loop/")
            || path_lower.contains("\\loop\\")
            || path_lower.contains("_loops/")
            || path_lower.contains("_loops\\")
            || path_lower.contains("_loop/")
            || path_lower.contains("_loop\\")
            || path_lower.contains(" loops/")
            || path_lower.contains(" loops\\")
            || path_lower.contains("/pads/")
            || path_lower.contains("\\pads\\")
            || path_lower.contains("/pad/")
            || path_lower.contains("\\pad\\")
            || path_lower.contains(" pads/")
            || path_lower.contains(" pads\\")
            || path_lower.contains("/synth pads/")
            || path_lower.contains("\\synth pads\\")
            || path_lower.contains("/leads/")
            || path_lower.contains("\\leads\\")
            || path_lower.contains("/lead/")
            || path_lower.contains("\\lead\\")
            || path_lower.contains("/arps/")
            || path_lower.contains("\\arps\\")
            || path_lower.contains("/arp/")
            || path_lower.contains("\\arp\\")
            || path_lower.contains("/synths/")
            || path_lower.contains("\\synths\\")
            || path_lower.contains("/synth/")
            || path_lower.contains("\\synth\\")
            || path_lower.contains("/bass/")
            || path_lower.contains("\\bass\\")
            || path_lower.contains("/basslines/")
            || path_lower.contains("\\basslines\\")
            || path_lower.contains("/melodic/")
            || path_lower.contains("\\melodic\\")
            || path_lower.contains("/music loops/")
            || path_lower.contains("\\music loops\\")
            || path_lower.contains("/atmosphere/")
            || path_lower.contains("\\atmosphere\\")
            || path_lower.contains("/atmospheres/")
            || path_lower.contains("\\atmospheres\\")
            || path_lower.contains("/drone/")
            || path_lower.contains("\\drone\\")
            || path_lower.contains("/drones/")
            || path_lower.contains("\\drones\\")
        {
            return true;
        }
        ...
        if name_lower.contains("_loop")
            || name_lower.contains("loop_")
            || name_lower.contains(" loop")
            || name_lower.contains("loop ")
            || name_lower.starts_with("loop")
            || name_lower.ends_with("loop")
            || name_lower.contains("-loop")
            || name_lower.contains("loop-")
        {
            return true;
...
fn pick_random_key() -> String {
    ...
    let conn = match open_dedicated_conn() {
        ...
        Err(e) => {
            write_app_log(format!(
                "[track_generator] pick_random_key: DB error: {}",
                e
            ));
    ...
    write_app_log(format!(
        "[track_generator] pick_random_key: selected '{}'",
        key
    ));
    key
}
...
const SOFT_PATTERNS: &[&str] = &[
    "soft", "smooth", "mellow", "gentle", "ambient", "chill", "deep", "minimal", "subtle", "warm",
    "lush", "dreamy", "ethereal", "delicate", "clean", "pure", "light", "airy", "floating",
    "serene", "calm",
];
...
fn ensure_blacklist_loaded() {
    ...
    if let Ok(entries) = crate::db::global().blacklist_list() {
        ...
        write_app_log(format!(
            "[track_generator] Loaded {} blacklist entries from DB",
...
fn ensure_whitelist_loaded() {
    ...
    if let Ok(entries) = crate::db::global().whitelist_list() {
        ...
        write_app_log(format!(
            "[track_generator] Loaded {} whitelist entries from DB",
...
fn is_path_whitelisted(path: &str) -> bool {
    ...
    if let Ok(whitelist) = DIRECTORY_WHITELIST.lock() {
        ...
        if !LOGGED.swap(true, Ordering::Relaxed) {
            ...
            write_app_log(format!(
                "[whitelist] Rejected path: {} (whitelist dirs: {:?})",
                path, dirs
            ));
        }
        false
...
pub fn clear_sample_blacklist() {
    ...
    if let Ok(mut blacklist) = GENERATION_BLACKLIST.lock() {
        ...
        blacklist.clear();
        write_app_log(format!(
            "[track_generator] Cleared sample blacklist ({} samples)",
            count
        ));
    }
    // Clear from DB
...
pub fn add_to_whitelist(path: &str) {
    ...
    write_app_log(format!(
        "[track_generator] Added to whitelist: {}",
        normalized
    ));
}
...
pub fn remove_from_whitelist(path: &str) -> bool {
    ...
    if removed {
        write_app_log(format!(
            "[track_generator] Removed from whitelist: {}",
            path
        ));
    }
    removed
...
pub fn clear_whitelist() {
    ...
    if let Ok(mut whitelist) = DIRECTORY_WHITELIST.lock() {
        ...
        whitelist.clear();
        write_app_log(format!(
            "[track_generator] Cleared directory whitelist ({} directories)",
            count
        ));
    }
    // Clear from DB
...
fn query_samples_with_key(
    ...
    if results.is_empty() && key.is_some() {
        write_app_log(format!(
            "[track_generator] {}: No samples with key in filename - track will be empty",
            label
        ));
    }
...
fn query_samples_internal(
    ...
    write_app_log(format!(
        "[track_generator] {}: patterns=[{}] key={:?} require_loop={}",
        label,
    ...
    let conn = match open_dedicated_conn() {
        ...
        Err(e) => {
            write_app_log(format!(
                "[track_generator] query_samples_internal: DB error: {}",
                e
            ));
            return vec![];
        }
        ...
        Some(k) => {
            ...
            if parts.len() == 2 {
                ...
                crate::als_project::get_compatible_keys(
                    root,
                    if quality.eq_ignore_ascii_case("minor") {
                        "Aeolian"
                    } else {
                        "Ionian"
                    },
    ...
    let mut stmt = match conn.prepare(&query) {
        ...
        Err(e) => {
            write_app_log(format!(
                "[track_generator] query_samples_internal: prepare error: {}",
                e
            ));
            return vec![];
        }
    ...
    let all_samples: Vec<SampleInfo> = stmt
        .query_map([], |row| {
        ...
        .map(|rows| {
            rows.filter_map(|r| r.ok())
                .map(
                    |(path, duration, bpm, size, sample_rate, channels, bits_per_sample)| {
                        SampleInfo::from_db(
                            &path,
                            duration,
                            size,
    ...
    use crate::sample_filters::{
        BAD_GENRES, CONSTRUCTION_KIT_KEYWORDS, PROJECT_RENDER_KEYWORDS, REVERSED_SUFFIXES,
        is_ableton_project_sample, is_excluded_genre,
    };
    ...
    let all_samples: Vec<SampleInfo> = all_samples
        ...
        .filter(|s| {
            ...
            let has_pattern = include_patterns
                .iter()
            ...
            // Skip reversed files
            if REVERSED_SUFFIXES
                .iter()
                .any(|suffix| s.path.ends_with(suffix))
            {
                return false;
            }
            // Skip frozen/consolidated/rendered files
            if PROJECT_RENDER_KEYWORDS
                .iter()
        ...
        loop_filtered
            ...
            .filter(|s| {
                ...
                if let Some(parsed_key) = crate::sample_analysis::extract_key(&s.path) {
                    ...
                    compatible_keys
                        .iter()
                        .any(|ck| ck.eq_ignore_ascii_case(&parsed_key))
                } else {
    ...
    let mut scored: Vec<(SampleInfo, f32)> = key_filtered
        ...
        .map(|s| {
            ...
            let hard_matches = HARD_PATTERNS
                .iter()
                .filter(|p| path_lower.contains(*p))
                .count() as f32;

            // Count soft pattern matches
            let soft_matches = SOFT_PATTERNS
                .iter()
                .filter(|p| path_lower.contains(*p))
                .count() as f32;
...
fn load_song_samples(
    ...
    write_app_log(format!(
        "[track_generator] load_song_samples: song {} starting, target_key={:?}, atonal={}",
        song_num, target_key, atonal
    ));
    // When global atonal is set, all types are atonal
    let track_key = target_key
        .map(|k| k.to_string())
        .unwrap_or_else(pick_random_key);
    let key_filter: Option<&str> = if atonal { None } else { Some(&track_key) };
    write_app_log(format!(
        "[track_generator] load_song_samples: song {} using key={}, key_filter={:?} (took {:?})",
        song_num,
        track_key,
        key_filter,
        start.elapsed()
    ...
    let total_samples: u32 = track_counts.kick
        + track_counts.kick_rumble
        + track_counts.kick_noise
        + track_counts.hardcore_kick
        + track_counts.clap
        + track_counts.snare
        + track_counts.hat
        + track_counts.perc
        ...
        + track_counts.arp
        + track_counts.riser
        + track_counts.downlifter
        + track_counts.crash
        + track_counts.impact
        + track_counts.hit
        + track_counts.sweep_up
        + track_counts.sweep_down
        + track_counts.snare_roll
        + track_counts.reverse
        + track_counts.sub_drop
        + track_counts.boom_kick
        + track_counts.atmos
        + track_counts.glitch
        + track_counts.scatter
        + track_counts.vox;
    ...
    macro_rules! query_n_keyed {
        ($label:expr, $inc:expr, $require_loop:expr, $count:expr, $key:expr) => {{
            ...
            for i in 0..$count as usize {
                ...
                progress(&format!(
                    "SAMPLE_PROGRESS:{}:{}",
                    samples_searched, total_samples
                ));
    ...
    let kicks = query_n_keyed!(
        "KICK",
        kick_inc,
        true,
        track_counts.kick,
        key_for(type_atonal.kick)
    );
    ...
    let kick_rumbles = query_n_keyed!(
        "KICK RUMBLE",
        rumble_inc,
        true,
        track_counts.kick_rumble,
        key_for(type_atonal.kick_rumble)
    );
    ...
    let kick_noises = query_n_keyed!(
        "KICK NOISE",
        noise_inc,
        true,
        track_counts.kick_noise,
        key_for(type_atonal.kick_noise)
    );
    ...
    let hardcore_kicks = query_n_keyed!(
        "HARDCORE KICK",
        hc_kick_inc,
        true,
        track_counts.hardcore_kick,
        key_for(type_atonal.hardcore_kick)
    );

    let clap_inc = &["clap_loop", "clap loop", "claps/", "clap/"];
    let claps = query_n_keyed!(
        "CLAP",
        clap_inc,
        true,
        track_counts.clap,
        key_for(type_atonal.clap)
    );

    // Exclude snare_roll/snare_build - those go to SNARE_ROLL
    let snare_inc = &["snare_loop", "snare loop", "snares/", "snare/"];
    let snares = query_n_keyed!(
        "SNARE",
        snare_inc,
        true,
        track_counts.snare,
        key_for(type_atonal.snare)
    );
    ...
    let hats = query_n_keyed!(
        "HAT",
        hat_inc,
        true,
        track_counts.hat,
        key_for(type_atonal.hat)
    );
    ...
    let percs = query_n_keyed!(
        "PERC",
        perc_inc,
        true,
        track_counts.perc,
        key_for(type_atonal.perc)
    );
    ...
    let rides = query_n_keyed!(
        "RIDE",
        ride_inc,
        true,
        track_counts.ride,
        key_for(type_atonal.ride)
    );
    ...
    let fills = query_n_keyed!(
        "FILL",
        fill_inc,
        false,
        track_counts.fill,
        key_for(type_atonal.fill)
    );
    ...
    let breakbeats = query_n_keyed!(
        "BREAKBEAT",
        breakbeat_inc,
        true,
        track_counts.breakbeat,
        key_for(type_atonal.breakbeat)
    );
    ...
    let (basses, subs, pads, leads, synths, arps, keyss): (
        Vec<Vec<SampleInfo>>,
        Vec<Vec<SampleInfo>>,
        Vec<Vec<SampleInfo>>,
        Vec<Vec<SampleInfo>>,
        ...
        let basses = query_n_keyed!(
            "BASS",
            bass_inc,
            true,
            track_counts.bass,
            key_for(type_atonal.bass)
        );

        let sub_inc = &["sub_loop", "sub loop", "sub_bass", "808_loop", "808 loop"];
        let subs = query_n_keyed!(
            "SUB",
            sub_inc,
            true,
            track_counts.sub,
            key_for(type_atonal.sub)
        );
        ...
        let pads = query_n_keyed!(
            "PAD",
            pad_inc,
            true,
            track_counts.pad,
            key_for(type_atonal.pad)
        );

        let lead_inc = &["lead_loop", "lead loop", "synth_lead", "lead/"];
        let leads = query_n_keyed!(
            "LEAD",
            lead_inc,
            true,
            track_counts.lead,
            key_for(type_atonal.lead)
        );
        ...
        let synths = query_n_keyed!(
            "SYNTH",
            synth_inc,
            true,
            track_counts.synth,
            key_for(type_atonal.synth)
        );
        ...
        let arps = query_n_keyed!(
            "ARP",
            arp_inc,
            true,
            track_counts.arp,
            key_for(type_atonal.arp)
        );
        ...
        let keyss = query_n_keyed!(
            "KEYS",
            keys_inc,
            true,
            track_counts.keys,
            key_for(type_atonal.keys)
        );
    ...
    let hits = query_n_keyed!(
        "HIT",
        hit_inc,
        false,
        track_counts.hit,
        key_for(type_atonal.hit)
    );
    ...
    let sweep_ups = query_n_keyed!(
        "SWEEP UP",
        sweep_up_inc,
        false,
        track_counts.sweep_up,
        key_for(type_atonal.sweep_up)
    );
    ...
    let sweep_downs = query_n_keyed!(
        "SWEEP DOWN",
        sweep_down_inc,
        false,
        track_counts.sweep_down,
        key_for(type_atonal.sweep_down)
    );
    ...
    let snare_rolls = query_n_keyed!(
        "SNARE_ROLL",
        snare_roll_inc,
        false,
        track_counts.snare_roll,
        key_for(type_atonal.snare_roll)
    );
    ...
    let reverses = query_n_keyed!(
        "REVERSE",
        reverse_inc,
        false,
        track_counts.reverse,
        key_for(type_atonal.reverse)
    );
    ...
    let sub_drops = query_n_keyed!(
        "SUB_DROP",
        sub_drop_inc,
        false,
        track_counts.sub_drop,
        key_for(type_atonal.sub_drop)
    );
    ...
    let boom_kicks = query_n_keyed!(
        "BOOM_KICK",
        boom_kick_inc,
        false,
        track_counts.boom_kick,
        key_for(type_atonal.boom_kick)
    );

    let atmoses = if midi_tracks {
        vec![]
    } else {
        let atmos_inc = &[
            "atmos",
    ...
    let glitches = query_n_keyed!(
        "GLITCH",
        glitch_inc,
        false,
        track_counts.glitch,
        key_for(type_atonal.glitch)
    );
    ...
    let scatters = {
        ...
        for i in 0..track_counts.scatter as usize {
            ...
            progress(&format!("Searching {}...", track_label));
            let samples =
                query_samples_with_key_optional(&track_label, scatter_inc, false, 1, scatter_key);
            if !samples.is_empty() {
                progress(&format!("Found {}: {}", track_label, samples[0].name));
            }
            results.push(samples);
    ...
    let voxes = query_n_keyed!(
        "VOX",
        vox_inc,
        false,
        track_counts.vox,
        key_for(type_atonal.vox)
    );
    ...
    write_app_log(format!(
        ...
        song_num,
        start.elapsed(),
        count_nonempty(&basses),
        basses.len(),
        count_nonempty(&leads),
        leads.len(),
        count_nonempty(&pads),
        pads.len()
    ));
...
pub struct TypeAtonal {
    ...
    pub kick: bool,
    pub kick_rumble: bool,
    pub kick_noise: bool,
    pub hardcore_kick: bool,
    pub clap: bool,
    pub snare: bool,
    pub hat: bool,
    pub perc: bool,
...
fn generate_inner(
    ...
    write_app_log(format!(
        "[track_generator] generate: target_key={:?}",
        target_key
    ));
    ...
    for song_num in 1..=num_songs {
        ...
        progress(&format!(
            "Loading samples for song {}/{}...",
            song_num, num_songs
        ));
        write_app_log(format!(
            "[track_generator] generate: calling load_song_samples({}) with hardness={}, track_counts={:?}",
            song_num, hardness, track_counts
        ));
        let song_samples = load_song_samples(
            song_num,
            target_key.as_deref(),
            atonal,
            hardness,
        ...
        write_app_log(format!(
            "[track_generator] generate: load_song_samples({}) done",
            song_num
        ));
    }
    write_app_log(format!(
        "[track_generator] generate: sample loading complete, elapsed {:?}",
        gen_start.elapsed()
    ));
    ...
    let mut arrangements = get_arrangement_with_params(
        chaos,
        glitch_intensity,
        &section_overrides,
        density,
    ...
    let full_arrangement: Vec<(f64, f64)> = vec![
        ...
        (
            user_starts.breakdown.0 as f64,
            user_starts.breakdown.1 as f64,
        ),
    ...
    let find_arr = |name: &str| -> Vec<(f64, f64)> {
        ...
        if name.starts_with("DRUM LOOP ")
            || name.starts_with("BASS LOOP ")
            || name.starts_with("SYNTH LOOP ")
            || name.starts_with("PAD LOOP ")
        {
    ...
    let kicks_group_emitted =
        !song1.kicks.is_empty() || !song1.kick_rumbles.is_empty() || !song1.kick_noises.is_empty();
    let drums_group_emitted = !song1.claps.is_empty()
        || !song1.snares.is_empty()
        || !song1.hats.is_empty()
        || !song1.percs.is_empty()
        || !song1.rides.is_empty()
        || !song1.fills.is_empty()
        || !song1.breakbeats.is_empty()
        || !song1.hardcore_kicks.is_empty();
    ...
    let melodics_group_emitted = !song1.leads.is_empty()
        || !song1.synths.is_empty()
        || !song1.pads.is_empty()
        || !song1.arps.is_empty()
        || !song1.atmoses.is_empty();
    let fx_group_emitted = !song1.risers.is_empty()
        || !song1.downlifters.is_empty()
        || !song1.crashes.is_empty()
        || !song1.impacts.is_empty()
        || !song1.hits.is_empty()
        || !song1.sweep_ups.is_empty()
        || !song1.sweep_downs.is_empty()
        || !song1.snare_rolls.is_empty()
        || !song1.reverses.is_empty()
        || !song1.glitches.is_empty()
        || !song1.voxes.is_empty();
    ...
    let total_tracks = group_count
        + song1.kicks.len()
        + song1.kick_rumbles.len()
        + song1.kick_noises.len()
        + song1.hardcore_kicks.len()
        + song1.claps.len()
        + song1.snares.len()
        + song1.hats.len()
        + song1.percs.len()
    ...
    let has_requested = |samples: &[Vec<SampleInfo>]| -> bool { !samples.is_empty() };
    ...
    macro_rules! create_tracks {
        ($samples:expr, $base_name:expr, $color:expr, $group_id:expr) => {{
            ...
            for i in 0..$samples.len() {
                ...
                match create_arranged_track_multi(
    ...
    let kicks_has_children = has_requested(&song1.kicks)
        || has_requested(&song1.kick_rumbles)
        || has_requested(&song1.kick_noises);
    ...
    let drums_has_children = has_requested(&song1.claps)
        || has_requested(&song1.snares)
        || has_requested(&song1.hats)
        || has_requested(&song1.percs)
        || has_requested(&song1.rides)
        || has_requested(&song1.fills)
        || has_requested(&song1.breakbeats)
        || has_requested(&song1.hardcore_kicks);
    ...
    if midi_tracks {
        ...
        let midi_leads_group =
            create_group_track("LEADS", MELODICS_COLOR, midi_leads_group_id, &ids)?;
        ...
        if let Ok(midi_trks) = midi_result {
            ...
            for mt in &midi_trks {
                ...
                if gid == midi_bass_group_id {
                    bass_has = true;
                } else if gid == midi_leads_group_id {
                    leads_has = true;
                } else if gid == midi_pads_group_id {
                    pads_has = true;
                } else if gid == midi_keys_group_id {
                    keys_has = true;
                }
            }
        ...
        let melodics_has_children = has_requested(&song1.leads)
            || has_requested(&song1.synths)
            || has_requested(&song1.pads)
            || has_requested(&song1.arps)
            || has_requested(&song1.keyss)
            || has_requested(&song1.atmoses);
    ...
    let fx_has_children = has_requested(&song1.risers)
        || has_requested(&song1.downlifters)
        || has_requested(&song1.crashes)
        || has_requested(&song1.impacts)
        || has_requested(&song1.hits)
        || has_requested(&song1.sweep_ups)
        || has_requested(&song1.sweep_downs)
        || has_requested(&song1.snare_rolls)
        || has_requested(&song1.reverses)
        || has_requested(&song1.glitches)
        || has_requested(&song1.voxes);
    if fx_has_children {
    ...
    let clip_count: usize = all_tracks
        .iter()
        .map(|t| t.matches("<AudioClip").count() + t.matches("<MidiClip").count())
        .sum();
    ...
    if inner_locators_re.is_match(&xml) {
        xml = inner_locators_re
            .replace(&xml, locators_xml.as_str())
            .to_string();
        write_app_log(format!(
            "[track_generator] Inserted {} locators",
            locators_xml.matches("<Locator ").count()
        ));
    } else {
        write_app_log(
            "[track_generator] WARNING: Could not find inner <Locators /> placeholder in XML"
                .into(),
        );
    }
    ...
    xml = tempo_event_re
        .replace(
            &xml,
            format!(
                r#"<FloatEvent Id="0" Time="-63072000" Value="{}" />"#,
                bpm_str
            ),
        )
        .to_string();
    ...
    let output_name = output_path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("project.als");
    progress(&format!("Writing {}", output_name));
    write_app_log(format!(
        "[track_generator] Writing output: {:?}",
        output_path
    ));
    ...
    write_app_log(format!(
        "[track_generator] Completed: {:?} ({} tracks, {} clips)",
        output_path, track_count, clip_count
    ));
...
fn inject_master_chain(xml: &str, ids: &IdAllocator) -> String {
    ...
    let Some(m) = main_re.find(xml) else {
        return xml.to_string();
    };
    let main_start = m.start();
    let Some(rel_main_end) = xml[main_start..].find("</MainTrack>") else {
        return xml.to_string();
    };
    ...
    let Some(rel_dev) = main.find(devices_open) else {
        return xml.to_string();
    };
...
fn get_track_samples(song: &SongSamples, track_name: &str) -> Vec<SampleInfo> {
    ...
    // Try each type
    if let Some(idx) = parse_idx(track_name, "KICK RUMBLE") {
        return song.kick_rumbles.get(idx).cloned().unwrap_or_default();
    }
    if let Some(idx) = parse_idx(track_name, "KICK NOISE") {
        return song.kick_noises.get(idx).cloned().unwrap_or_default();
    }
    if let Some(idx) = parse_idx(track_name, "HARDCORE KICK") {
        return song.hardcore_kicks.get(idx).cloned().unwrap_or_default();
    }
    ...
    if let Some(idx) = parse_idx(track_name, "BREAKBEAT") {
        return song.breakbeats.get(idx).cloned().unwrap_or_default();
    }
...
fn create_arranged_track_multi(
    ...
    let volume_value = if name.starts_with("KICK") {
        "1"
    } else {
        "0.251188643"
    };
    track = volume_re
        .replace(&track, format!(r#"${{1}}{}${{2}}"#, volume_value))
        .to_string();
    ...
    if track.contains("<Events />") {
        ...
        track = events_re
            .replacen(&track, 1, new_events.as_str())
            .to_string();
    }
...
mod tests {
    ...
    fn test_apply_parallelism_per_section_effective_vals() {
        ...
        let result = apply_parallelism_per_section(
            arrangements.clone(),
            &p_vals,
            global_p,
            &v_vals,
    ...
    fn test_apply_glitch_edits_heavy_intensity() {
        ...
        assert_ne!(
            result, arrangements,
            "Heavy glitch should have modified the arrangement"
        );
    ...
    fn test_generate_scatter_hits_logic() {
        ...
        for track in results {
            for (start, _) in track.sections {
                assert!(
                    start >= 1.0 && start < 33.0,
                    "Scatter hit at {} outside intro",
                    start
                );
            }
        }
...
mod additional_tests {
    ...
    fn test_different_seeds_produce_different_output() {
        ...
        assert!(
            unique.len() >= 2,
            "5 distinct seeds must produce ≥2 distinct outputs"
        );
    }
    ...
    fn test_helper_works_without_generate_init() {
        ...
        assert!(
            !out.is_empty(),
            "helper should fall back when no seed is set"
        );
    }
    ...
    fn test_get_arrangement_basics() {
        ...
        let kick = arr
            .iter()
            .find(|t| t.name == "KICK")
            .expect("KICK track missing");
        assert!(kick.sections.len() >= 10); // Lots of gaps for fills

        let clap = arr
            .iter()
            .find(|t| t.name == "CLAP")
            .expect("CLAP track missing");
    ...
    fn test_apply_chaos_to_arrangements_protected() {
        let arrangements = vec![TrackArrangement::new(
            "FILL 1",
        ...
        assert_eq!(
            result, arrangements,
            "Protected FILL track should not be modified by chaos"
        );
    }
    ...
    fn test_apply_parallelism_exempt_tracks() {
        ...
        assert_eq!(
            result.len(),
            2,
            "Exempt IMPACT tracks should not be thinned by parallelism"
        );
    }
    ...
    fn test_apply_variation_zero() {
        ...
        assert_eq!(
            result, arrangements,
            "Zero variation should not modify arrangements"
        );
    }
    ...
    fn send_levels_lead_has_delay() {
        ...
        assert!(
            lead_dly > pad_dly,
            "lead delay ({}) should exceed pad delay ({})",
            lead_dly,
            pad_dly
        );
    }
    ...
    fn group_sidechain_compressor_template_is_preconfigured() {
        ...
        let cap = re
            .captures(thresh_slice)
            .expect("Threshold Manual not found");
        let thresh: f64 = cap[1].parse().unwrap();
        assert!(
            thresh < 0.5,
            "Threshold {} is too loose for sidechain ducking",
            thresh
        );
    }
    ...
    fn build_group_sidechain_substitutes_source_id_and_allocates_ids() {
        ...
        assert!(
            !out.contains(r#"Id="2""#),
            "template Id=2 should be reallocated"
        );
    }
    ...
    fn build_group_sidechain_twice_has_disjoint_ids() {
        ...
        for id in &ids_a {
            assert!(
                !ids_b.contains(id),
                "ID {} reused across two group compressors",
                id
            );
        }
    }
    ...
    fn inject_device_into_group_chain_replaces_empty_devices() {
        ...
        assert!(
            ...
            "empty Devices placeholder should be gone"
        );
        assert!(
            out.contains("<Devices>"),
            "Devices should now be a container"
        );
        assert!(out.contains("</Devices>"), "Devices container should close");
        assert!(
            out.contains(r#"<Compressor2 Id="99" />"#),
            "device content should be injected"
        );
        // Preserved structure.
    ...
    fn reallocate_device_ids_gives_unique_fresh_ids() {
        ...
        let id_re = regex::Regex::new(r#"Id="(\d+)""#).unwrap();
        let ids1: Vec<_> = id_re
            .captures_iter(&out1)
            .map(|c| c[1].to_string())
            .collect();
        let ids2: Vec<_> = id_re
            .captures_iter(&out2)
            .map(|c| c[1].to_string())
            .collect();
        for id in &ids1 {
    ...
    fn inject_master_chain_places_devices_before_existing_stereogain() {
        ...
        let eq8_pos = out[main_pos..]
            .find("<Eq8 ")
            .expect("Eq8 should be inside MainTrack");
        let stereogain_pos = out[main_pos..].find("<StereoGain").unwrap();
        let limiter_pos = out[main_pos..]
            .find("<Limiter ")
            .expect("Limiter should be inside MainTrack");
        assert!(eq8_pos < stereogain_pos, "Eq8 must precede StereoGain");
        assert!(
            limiter_pos < stereogain_pos,
            "Limiter must precede StereoGain"
        );
        assert!(
            eq8_pos < limiter_pos,
            "Eq8 must precede Limiter (HPF before peak catch)"
        );
        ...
        assert!(
            !at_slice.contains("<Limiter "),
            "Limiter leaked into AudioTrack"
        );
    }
    ...
    fn master_eq8_template_has_band0_hpf_at_30hz() {
        ...
        assert!(
            count >= 1,
            "Eq8 template must have at least one Manual=30 (Band.0 Freq)"
        );
        ...
        assert!(
            slice.contains(r#"<Manual Value="30" />"#),
            "Band.0 Freq Manual should be 30"
        );
        assert!(
            !slice.contains(r#"<Manual Value="10" />"#),
            "Band.0 Freq Manual should not be stock 10"
        );
    }
    ...
    fn master_limiter_template_has_safe_ceiling() {
        ...
        assert!(
            val >= -1.0,
            "Ceiling {} dB is too aggressive (< -1 dB)",
            val
        );
    }
    ...
    fn section_values_serde_is_flat_bar_keyed_map() {
        ...
        assert!(
            json.contains(r#""1":0.5"#),
            "missing flat bar-1 key: {}",
            json
        );
        assert!(
            json.contains(r#""9":0.25"#),
            "missing flat bar-9 key: {}",
            json
        );
        assert!(
            !json.contains(r#""blocks""#),
            "struct wrapper leaked: {}",
            json
        );
    ...
    fn apply_glitch_edits_gates_on_per_block_threshold() {
        ...
        assert_ne!(
            out, arr,
            "a pinned block > 0.05 must trigger glitch even with global=0"
        );
    }
    ...
    fn remap_clips_range_that_straddles_shrunk_section_end() {
        ...
        assert!(
            (r.1 - 81.0).abs() < 1e-9,
            "clipped to user breakdown end, got {}",
            r.1
        );
    }
