...
fn get_arrangement(chaos: f32) -> Vec<TrackArrangement> {
    ...
    let mut base = vec![
        ...
        TrackArrangement::new("KICK RUMBLE", vec![
        ...
        TrackArrangement::new("KICK NOISE", vec![
                // BUILD - enters bar 33
                (33.0, 40.5),
                (41.0, 48.0),
                (49.0, 56.75),
        ...
        TrackArrangement::new("HARDCORE KICK", vec![
        ...
        TrackArrangement::new("BREAKBEAT", vec![
...
fn load_song_samples(song_num: u32, target_key: Option<&str>, atonal: bool, hardness: f32, track_counts: &TrackCounts, type_atonal: &TypeAtonal, midi_tracks: bool, on_progress: Option<&dyn Fn(&str)>) -> SongSamples {
    ...
    let total_samples: u32 = track_counts.kick + track_counts.kick_rumble + track_counts.kick_noise + track_counts.hardcore_kick
        + track_counts.clap + track_counts.snare + track_counts.hat
        + track_counts.perc + track_counts.ride + track_counts.fill + track_counts.breakbeat
    ...
    let kick_inc = &["kick_loop", "kick loop", "drum_loops/kick", "drum loops/kick", "kick_punch", "kick punch"];
    let kicks = query_n_keyed!("KICK", kick_inc, true, track_counts.kick, key_for(type_atonal.kick));

    let rumble_inc = &["rumble", "kick_rumble", "kick rumble", "schranz_drive", "drive_loop", "rumble_bass"];
    let kick_rumbles = query_n_keyed!("KICK RUMBLE", rumble_inc, true, track_counts.kick_rumble, key_for(type_atonal.kick_rumble));

    let noise_inc = &["top_noise", "top noise", "noise_crunch", "kick_top", "kick top", "schranz_top"];
    let kick_noises = query_n_keyed!("KICK NOISE", noise_inc, true, track_counts.kick_noise, key_for(type_atonal.kick_noise));

    let hc_kick_inc = &["hardcore_kick", "hardcore kick", "hardstyle_kick", "hardstyle kick", "hard_kick", "distorted_kick"];
    let hardcore_kicks = query_n_keyed!("HARDCORE KICK", hc_kick_inc, true, track_counts.hardcore_kick, key_for(type_atonal.hardcore_kick));
    ...
    let fill_inc = &["drum_fill", "drum fill", "fills/", "fill/", "drum_break", "breaks/", "pre_drop", "pre drop"];
    let fills = query_n_keyed!("FILL", fill_inc, false, track_counts.fill, key_for(type_atonal.fill));

    let breakbeat_inc = &["breakbeat", "break_beat", "break beat", "breakbeats/", "amen", "jungle_break"];
    let breakbeats = query_n_keyed!("BREAKBEAT", breakbeat_inc, true, track_counts.breakbeat, key_for(type_atonal.breakbeat));
    ...
    SongSamples {
        key: track_key,
        kicks, kick_rumbles, kick_noises, hardcore_kicks,
        claps, snares, hats, percs, rides, fills, breakbeats,
        basses, subs,
...
impl Default for TrackCounts {
    fn default() -> Self {
        Self {
            kick: 1, kick_rumble: 1, kick_noise: 1, hardcore_kick: 1,
...
fn generate_inner(
    ...
    let kicks_group_emitted = !song1.kicks.is_empty() || !song1.kick_rumbles.is_empty()
        || !song1.kick_noises.is_empty();
    let drums_group_emitted = !song1.claps.is_empty() || !song1.snares.is_empty()
        ...
        || !song1.fills.is_empty() || !song1.breakbeats.is_empty() || !song1.hardcore_kicks.is_empty();
    ...
    let kicks_has_children = has_requested(&song1.kicks) || has_requested(&song1.kick_rumbles)
    ...
    create_tracks!(song1.kick_rumbles, "KICK RUMBLE", DRUMS_COLOR, if kicks_has_children { kicks_group_id as i32 } else { -1 });
    create_tracks!(song1.kick_noises, "KICK NOISE", DRUMS_COLOR, if kicks_has_children { kicks_group_id as i32 } else { -1 });
    ...
    let drums_has_children = has_requested(&song1.claps) || has_requested(&song1.snares) ||
                             ...
                             has_requested(&song1.fills) || has_requested(&song1.breakbeats) || has_requested(&song1.hardcore_kicks);
    ...
    create_tracks!(song1.breakbeats, "BREAKBEAT", DRUMS_COLOR, if drums_has_children { drums_group_id as i32 } else { -1 });
    create_tracks!(song1.hardcore_kicks, "HARDCORE KICK", DRUMS_COLOR, if drums_has_children { drums_group_id as i32 } else { -1 });
